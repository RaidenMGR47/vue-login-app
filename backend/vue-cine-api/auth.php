...
switch($method) {
    case 'POST':
        // Login o Register y otras acciones POST
        ...
        if ($action === 'login') {
            ...
            // Actualizar avatar del usuario (usa usuario en sesión)
    ...
    case 'DELETE':
        // Eliminar usuario: usa el usuario de sesión si existe, sino acepta username en el body
        $data = json_decode(file_get_contents("php://input"), true);
        $sessionUser = $_SESSION['username'] ?? null;
        // Si se envía 'username' en el body, usarlo como objetivo cuando no hay sesión
        $targetUser = $sessionUser ?? ($data['username'] ?? null);

        if (!$targetUser) {
            sendResponse(false, null, "No autenticado o usuario no especificado");
        }
        ...
        if (!empty($data['action']) && $data['action'] === 'delete') {
            try {
                ...
                $stmt1->bindParam(":username", $targetUser);
                $stmt1->execute();
                ...
                $stmt2->bindParam(":username", $targetUser);
                $stmt2->execute();

                if ($stmt2->rowCount() > 0) {
                    $db->commit();
                    // Si borramos al usuario de la sesión, limpiar la sesión
